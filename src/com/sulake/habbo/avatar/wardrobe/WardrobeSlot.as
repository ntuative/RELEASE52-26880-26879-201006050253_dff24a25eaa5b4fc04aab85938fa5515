package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit
   {
      ...
      private var var_324:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_324 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_451 && var_1399)
         {
            _loc5_ = var_29.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_642);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_38,parseInt(FigureData.const_717));
         ...
         if(var_324)
         {
            if(false)
            {
               var_324.bitmap.dispose();
            }
            var_324.bitmap = new BitmapData(var_324.width,var_324.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_324.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_451 = var_29.figureData.getFigureString();
               var_642 = var_29.gender;
               var_29.handler.saveWardrobeOutfit(var_1650,this);
               updateView();
               break;
            case "get_button":
               if(var_451)
               {
                  var_29.loadAvatarInEditor(var_451,var_642,var_29.clubMemberLevel);
               }
         }
      }
   }
